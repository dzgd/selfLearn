...
class MySpider:
    ...
    def startUp(self, url, key):
        ...
        try:
            ...
            pass
        self.cursor.execute("create table phones (mNo  varchar(32) primary key, mMark varchar(256),mPrice varchar(32),mNote varchar(1024))")
    def processSpider(self):
        try:
            ...
            for li in lis:
                ...
                self.cursor.execute(
                "insert into phones (mNo,mMark,mPrice,mNote) values (%s,%s,%s,%s)",
                (no, mark, price,note))
                self.con.commit()
                time.sleep(2)
                if no >= 50:
